...
?>
										...
										<?php
										$incomeQuery=$db->query("SELECT SUM(amount) AS IncomeSum FROM incomes WHERE user_id='$userID' AND MONTH(date_of_income)=MONTH(CURDATE()) AND YEAR(date_of_income)=YEAR(CURDATE())");
...
?>
							...
							<div id="wydatki">
								<div id="tabelaWydatki" class="col-lg-6 d-inline-block align-top">
									...
									<?php
										$expenseQuery=$db->query("SELECT SUM(amount) AS ExpenseSum FROM expenses WHERE user_id='$userID' AND MONTH(date_of_expense)=MONTH(CURDATE()) AND YEAR(date_of_expense)=YEAR(CURDATE())");
